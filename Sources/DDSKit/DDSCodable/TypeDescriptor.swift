...
public struct DDSTypeDescriptor: Sendable, CustomStringConvertible {
    ...
    public init(lookup name: String) throws(LookupError) {
    ...
    /// An error that occurs when a type lookup fails.
    public struct LookupError: Error {
        /// The name of the type that failed to be looked up.
        public let name: String
    }
...
public class DDSTypeBuilder {
    ...
    public func build() -> DDSTypeDescriptor {
        var name = std.string()
        ...
        switch ret {
            ...
            case eprosima.fastdds.dds.RETCODE_BAD_PARAMETER:
                fatalError("Failed to build DDS type: Another type with the same name already exists: \(name). This name is only local, so it's best to just make it more unique.")
            default:
